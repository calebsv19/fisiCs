        ...
        if (field->varDecl.varCount == 0) {
            ...
            if (anonInfo.category == TYPEINFO_STRUCT || anonInfo.category == TYPEINFO_UNION) {
                ...
                if (!nested && anonType->inlineStructOrUnionDef) {
                    nested = lookupFieldTypeInAggregateDefForInitValidation(anonType->inlineStructOrUnionDef,
                                                                            fieldName,
                                                                            scope);
                }
                if (nested) {
...
static void validateAggregateDesignatedFieldsRecursive(const ParsedType* aggregateType,
    ...
    for (size_t i = 0; i < compoundExpr->compoundLiteral.entryCount; ++i) {
        ...
        const ParsedType* fieldType =
            lookupAggregateFieldTypeForInitValidation(aggregateType, aggregateInfo, entry->fieldName, scope);
        if (!fieldType) {
            continue;
        }
