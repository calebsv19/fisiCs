...
Quick smoke fixtures that exercise the public CLI compile/link path without
depending on the makefile examples targets. Outputs land in `compilation/out/`
so they can be safely ignored.

## Single-file (no headers)

```bash
./fisics compilation/single.c -o compilation/out/single_bin
