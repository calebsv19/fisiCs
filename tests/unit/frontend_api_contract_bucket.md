...
| `frontend_api_contract_diagnostic_metadata` | Validate stable diagnostics metadata helpers for severity/category/code/stage names across parser, semantic, preprocessor, build/link, lexer, and extension lanes | pass |
...
| `frontend_api_contract_overlay_memory_check` | Validate the memory-check overlay bit parses independently, remains outside `overlay=all` for Slice 0, and does not pollute the physics-units helper | pass |
...
| `frontend_api_contract_units_attachment_local_export` | Validate resolved local/parameter declaration unit attachments export with source range navigation and no fake stable symbol ids | pass |
