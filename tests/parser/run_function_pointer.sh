...
if ! grep -Fq "FUNCTION DECLARATION" "$TMP_OUTPUT" && ! grep -Fq "FUNCTION_POINTER" "$TMP_OUTPUT"; then
  if ! grep -Fq "VAR_DECL" "$TMP_OUTPUT" || { ! grep -Fq "(*fn)" "$TMP_OUTPUT" && ! grep -Fq "TYPE: I (*)(void)" "$TMP_OUTPUT" && ! grep -Fq "TYPE: I* (void)" "$TMP_OUTPUT" && ! grep -Fq "TYPE: I (*)(void) (void)" "$TMP_OUTPUT"; }; then
