...
compiler_status=0
ENABLE_GNU_STATEMENT_EXPRESSIONS=0 "$BIN" tests/parser/gnu_statement_expr.c > "$TMP_OUTPUT" 2>&1 || compiler_status=$?
if [ "$compiler_status" -ne 1 ]; then
  echo "Expected compiler exit 1 when statement expressions are disabled; got $compiler_status" >&2
  cat "$TMP_OUTPUT" >&2
  exit 1
fi
