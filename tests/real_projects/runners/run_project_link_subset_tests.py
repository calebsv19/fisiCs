...
DEFAULT_STAGE_KEY = "B_link_subset"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run real-project minimal link-subset validation (Stage B)."
    )
    ...
    parser.add_argument("--limit", type=int, default=0, help="Limit number of targets.")
...
def run_stage_b(
    ...
    if not all_targets:
        raise RuntimeError("stage B has no targets configured in manifest")
    ...
    if not targets:
        raise RuntimeError("no stage B targets selected")
    ...
    with tempfile.TemporaryDirectory(prefix="fisics-realproj-stageb-") as tmp:
        ...
        for target in targets:
            ...
            if fisics_compile_ok:
                ...
                fisics_link_cmd = ["clang"] + [str(obj) for obj in fisics_objects] + target_link_args + [
                    "-o",
                    str(fisics_bin_out),
                ]
            ...
            if not args.skip_clang:
                ...
                if clang_compile_ok:
                    ...
                    clang_link_cmd = ["clang"] + [str(obj) for obj in clang_objects] + target_link_args + [
                        "-o",
                        str(clang_bin_out),
                    ]
...
def main() -> int:
    ...
    try:
        ...
        report = run_stage_b(project=project, project_root=project_root, args=args)
