...
DEFAULT_STAGE_KEY = "E_golden_behavior"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run real-project golden behavior validation (Stage E)."
    )
    ...
    parser.add_argument("--limit", type=int, default=0, help="Limit number of golden targets.")
...
def run_stage_e(
    ...
    default_expected_stdout_sha256 = str(stage_cfg.get("expected_stdout_sha256", "")).strip()
    default_expected_stderr_sha256 = str(stage_cfg.get("expected_stderr_sha256", "")).strip()
    default_expected_stdout_contains = list(stage_cfg.get("expected_stdout_contains", []))
    default_expected_stderr_contains = list(stage_cfg.get("expected_stderr_contains", []))
    ...
    if not all_targets:
        raise RuntimeError("stage E has no targets configured in manifest")
    ...
    if not targets:
        raise RuntimeError("no stage E targets selected")
    ...
    with tempfile.TemporaryDirectory(prefix="fisics-realproj-stagee-") as tmp:
        ...
        for target_index, target in enumerate(targets, start=1):
            ...
            if not input_paths and build_target is not None:
                input_paths = expand_target_inputs(project_root, build_target)
            if not input_paths:
                raise RuntimeError(f"stage E target '{target_id}' missing inputs")
            for p in input_paths:
                if not p.exists() or not p.is_file():
                    raise RuntimeError(f"stage E target '{target_id}' input missing: {p}")
            ...
            if (build_target is not None and build_target.get("use_llvm_core_link_args", False)) or target.get(
            ...
            for p in link_inputs:
                if not p.exists():
                    raise RuntimeError(f"stage E target '{target_id}' link input missing: {p}")
            ...
            if not run_cwd.exists() or not run_cwd.is_dir():
                raise RuntimeError(f"stage E target '{target_id}' run cwd missing: {run_cwd}")
            expected_exit_code = int(target.get("expected_exit_code", default_expected_exit_code))
            expected_stdout_sha256 = str(
                target.get("expected_stdout_sha256", default_expected_stdout_sha256)
            ).strip()
            expected_stderr_sha256 = str(
                target.get("expected_stderr_sha256", default_expected_stderr_sha256)
            ).strip()
            expected_stdout_contains = (
                default_expected_stdout_contains + list(target.get("expected_stdout_contains", []))
            )
            expected_stderr_contains = (
                default_expected_stderr_contains + list(target.get("expected_stderr_contains", []))
            )
            ...
            fisics_run_ok = False
            fisics_golden_ok = False
            fisics_stdout_sha256 = ""
            fisics_stderr_sha256 = ""
            fisics_link_ms = 0
            fisics_run_ms = 0
            if fisics_compile_ok:
                ...
                if fisics_link_ok:
                    ...
                    stdout_norm = normalize_output(fisics_run_result.stdout)
                    stderr_norm = normalize_output(fisics_run_result.stderr)
                    fisics_stdout_sha256 = sha256_text(stdout_norm)
                    fisics_stderr_sha256 = sha256_text(stderr_norm)
                    stdout_hash_ok = (not expected_stdout_sha256) or (
                        fisics_stdout_sha256 == expected_stdout_sha256
                    )
                    stderr_hash_ok = (not expected_stderr_sha256) or (
                        fisics_stderr_sha256 == expected_stderr_sha256
                    )
                    stdout_contains_ok = contains_all(stdout_norm, expected_stdout_contains)
                    stderr_contains_ok = contains_all(stderr_norm, expected_stderr_contains)
                    fisics_golden_ok = (
                        fisics_run_ok
                        and stdout_hash_ok
                        and stderr_hash_ok
                        and stdout_contains_ok
                        and stderr_contains_ok
                    )
            ...
            clang_golden_ok: bool | None = None
            clang_stdout_sha256 = ""
            clang_stderr_sha256 = ""
            clang_compile_ms = 0
            clang_link_ms = 0
            ...
            if not args.skip_clang:
                clang_compile_cmd = ["clang"] + pp_flags + target_clang_args
                ...
                if clang_compile_ok:
                    ...
                    if clang_link_ok:
                        ...
                        stdout_norm = normalize_output(clang_run_result.stdout)
                        stderr_norm = normalize_output(clang_run_result.stderr)
                        clang_stdout_sha256 = sha256_text(stdout_norm)
                        clang_stderr_sha256 = sha256_text(stderr_norm)
                        stdout_hash_ok = (not expected_stdout_sha256) or (
                            clang_stdout_sha256 == expected_stdout_sha256
                        )
                        stderr_hash_ok = (not expected_stderr_sha256) or (
                            clang_stderr_sha256 == expected_stderr_sha256
                        )
                        stdout_contains_ok = contains_all(stdout_norm, expected_stdout_contains)
                        stderr_contains_ok = contains_all(stderr_norm, expected_stderr_contains)
                        clang_golden_ok = (
            ...
            fisics_ok = fisics_compile_ok and fisics_link_ok and fisics_golden_ok
            clang_ok = None
            if not args.skip_clang:
                clang_ok = bool(clang_compile_ok and clang_link_ok and clang_golden_ok)
            ...
            results.append(
                {
                    ...
                    "fisics": {
                        ...
                        "golden_ok": fisics_golden_ok,
                        "failure_phase": failure_phase,
                        "failure_class": failure_class,
                        "golden": {
                            "stdout_sha256": fisics_stdout_sha256,
                            "stderr_sha256": fisics_stderr_sha256,
                        },
                        "timing_ms": {
                    ...
                    if args.skip_clang
                        ...
                        "ok": bool(clang_compile_ok and clang_link_ok and clang_golden_ok),
                        "compile_ok": bool(clang_compile_ok),
                        "link_ok": bool(clang_link_ok),
                        "run_ok": bool(clang_run_ok),
                        "golden_ok": bool(clang_golden_ok),
                        "golden": {
                            "stdout_sha256": clang_stdout_sha256,
                            "stderr_sha256": clang_stderr_sha256,
                        },
                        "timing_ms": {
    ...
    report = {
        ...
        "report_contract": build_report_contract(
            ...
            available_count=len(all_targets),
            selector={"target": args.target, "limit": args.limit},
            lane_flags={
                "clang_parity_enabled": not args.skip_clang,
                "dry_run": args.dry_run,
            },
...
def main() -> int:
    ...
    try:
        ...
        report = run_stage_e(project=project, project_root=project_root, args=args)
