...
DEFAULT_STAGE_KEY = "C_full_build"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run real-project full-build validation (Stage C)."
    )
    ...
    parser.add_argument("--limit", type=int, default=0, help="Limit number of full-build targets.")
...
def run_stage_c(
    ...
    timeout_sec = args.timeout_sec or int(stage_cfg.get("timeout_sec", 60))
    ...
    if not all_targets:
        raise RuntimeError("stage C has no targets configured in manifest")
    ...
    if not targets:
        raise RuntimeError("no stage C targets selected")
    ...
    with tempfile.TemporaryDirectory(prefix="fisics-realproj-stageb-") as tmp:
        tmp_root = Path(tmp)
        for target in targets:
            target_id = str(target.get("id", "")).strip()
            if not target_id:
                raise RuntimeError("stage C target missing id")
            input_paths = expand_target_inputs(project_root, target)
            if not input_paths:
                raise RuntimeError(f"stage C target '{target_id}' selected zero inputs")
            for p in input_paths:
                if not p.exists() or not p.is_file():
                    raise RuntimeError(f"stage C target '{target_id}' input missing: {p}")
            ...
            raw_link_inputs = default_link_inputs + list(target.get("link_inputs", []))
            link_inputs = [resolve_input_path(project_root, raw) for raw in raw_link_inputs]
            for p in link_inputs:
                if not p.exists():
                    raise RuntimeError(f"stage C target '{target_id}' link input missing: {p}")
            ...
            fisics_ok = fisics_compile_ok and fisics_link_ok
            clang_ok = None
            if not args.skip_clang:
                clang_ok = bool(clang_compile_ok and clang_link_ok)
            ...
            results.append(
                {
                    ...
                    "fisics": {
                        ...
                        "timing_ms": {
                            ...
                            "link": fisics_link_ms,
                            "total": fisics_compile_ms + fisics_link_ms,
                        },
                    },
                    "clang": None
                    if args.skip_clang
                    else {
                        "ok": bool(clang_compile_ok and clang_link_ok),
                        "compile_ok": bool(clang_compile_ok),
                        "link_ok": bool(clang_link_ok),
                        "timing_ms": {
                            "compile_total": clang_compile_ms,
                            "link": clang_link_ms,
                            "total": clang_compile_ms + clang_link_ms,
                        },
                    },
                }
            )
...
def main() -> int:
    ...
    try:
        ...
        report = run_stage_c(project=project, project_root=project_root, args=args)
