...
int main(void) {
    ...
    rs14_commit(s0, g0, 13u) ? (ok += 1u) : (reject += 1u);

    (void)rs14_reserve(&s1, &g1);
    rs14_commit(s1, g1, 17u) ? (ok += 1u) : (reject += 1u);

    (void)rs14_reserve(&s2, &g2);
    rs14_commit(s2, g2, 19u) ? (ok += 1u) : (reject += 1u);

    (void)rs14_reserve(&s3, &g3);
    rs14_abort(s3, g3) ? (ok += 1u) : (reject += 1u);

    (void)rs14_reserve(&s0, &g0);
    rs14_commit(s0, g0, 23u) ? (ok += 1u) : (reject += 1u);

    (void)rs14_reserve(&s1, &g1);
    rs14_abort(s1, g1) ? (ok += 1u) : (reject += 1u);

    (void)rs14_reserve(&s2, &g2);
    rs14_commit(s2, g2, 29u) ? (ok += 1u) : (reject += 1u);

    (void)rs14_reserve(&s3, &g3);
    rs14_commit(s3, g3, 31u) ? (ok += 1u) : (reject += 1u);
