...
struct Axis3W15Wide axis3_w15_wide_collect(
    ...
    va_list ap;
    struct Axis3W15Wide out;
    static const int lane_map[23] = {2, 0, 1, 2, 1, 0, 2, 0, 1, 2, 1, 0, 1, 2, 0, 2, 1, 0, 2, 1, 0, 2, 1};
    int i;

    out.lo = seed_lo + (long double)(epoch & 63u) * 0.0234375L;
    out.mid = seed_mid + (long double)(frontier & 63u) * 0.0107421875L;
    out.hi = seed_hi - (long double)(replay & 63u) * 0.001220703125L;
    out.tag = (epoch ^ frontier ^ replay) + rotl32(0x9e3779b9u ^ replay, (frontier ^ replay ^ epoch) & 31u);
    ...
    for (i = 0; i < count; ++i) {
        ...
        long double s = 0.005859375L * (long double)(i + 1);
        ...
        if (kind == 0) {
            ...
            out.tag ^= q10(ldv) + (unsigned)(i * 47 + 19);
        } else if (kind == 1) {
            int iv = va_arg(ap, int);
            out.lo += (long double)((iv + lane + i) & 63) * 0.005859375L;
            out.mid -= (long double)((iv >> 1) & 31) * 0.00244140625L;
            out.hi += (long double)((iv >> 2) & 31) * 0.002685546875L;
            out.tag = rotl32(out.tag ^ (unsigned)(iv + i * 53), 9u);
        } else {
            unsigned uv = va_arg(ap, unsigned);
            out.lo += (long double)((uv ^ epoch ^ replay) & 63u) * 0.004150390625L;
            out.mid += (long double)((uv >> 2) & 31u) * 0.00146484375L;
            out.hi -= (long double)((uv >> 3) & 31u) * 0.001220703125L;
            out.tag ^= rotl32(
                uv + epoch + frontier + replay + (unsigned)(i * 43 + 13),
                (unsigned)((i & 7) + 6)
            );
        }
    }
    ...
    out.lo += (long double)(rotl32(epoch ^ frontier, replay & 31u) & 63u) * 0.0078125L;
    out.mid += (long double)(rotl32(frontier ^ replay, epoch & 31u) & 63u) * 0.0068359375L;
