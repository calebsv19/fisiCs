...
struct Axis3W6Wide axis3_w6_wide_collect(
    ...
    va_list ap;
    struct Axis3W6Wide out;
    static const int lane_map[6] = {2, 1, 0, 2, 1, 0};
    int i;

    out.lo = seed_lo + (long double)(epoch & 7u) * 0.125L;
    out.mid = seed_mid;
    out.hi = seed_hi;
    out.tag = epoch ^ rotl32(0x9e3779b9u, frontier & 31u);

    va_start(ap, count);
    for (i = 0; i < count; ++i) {
        int kind = va_arg(ap, int);
        int lane = lane_map[(i + (int)(frontier & 3u)) % 6];
        ...
        if (kind == 0) {
            ...
            out.tag ^= rotl32(uv + frontier + (unsigned)(i * 13 + 5), (unsigned)((i & 7) + 4));
        }
    }
    va_end(ap);

    out.lo += (long double)(rotl32(epoch, frontier & 31u) & 31u) * 0.015625L;
    out.hi += (long double)((frontier >> 2) & 15u) * 0.0078125L;
    return out;
}
