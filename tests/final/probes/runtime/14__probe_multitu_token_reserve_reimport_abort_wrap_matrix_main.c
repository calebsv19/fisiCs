...
int main(void) {
    ...
    rs14_commit(s0, g0, 15u) ? (ok += 1u) : (reject += 1u);

    (void)rs14_reserve(&s1, &g1);
    rs14_abort(s1, g1) ? (ok += 1u) : (reject += 1u);

    (void)rs14_reserve(&s2, &g2);
    rs14_commit(s2, g2, 21u) ? (ok += 1u) : (reject += 1u);

    (void)rs14_reserve(&s3, &g3);
    rs14_commit(s3, g3, 27u) ? (ok += 1u) : (reject += 1u);

    (void)rs14_reserve(&s0, &g0);
    rs14_abort(s0, g0) ? (ok += 1u) : (reject += 1u);

    (void)rs14_reserve(&s1, &g1);
    rs14_commit(s1, g1, 33u) ? (ok += 1u) : (reject += 1u);

    (void)rs14_reserve(&s2, &g2);
    rs14_abort(s2, g2) ? (ok += 1u) : (reject += 1u);

    (void)rs14_reserve(&s3, &g3);
    rs14_commit(s3, g3, 39u) ? (ok += 1u) : (reject += 1u);
