...
int main(void) {
    ...
    (void)tg14_enqueue(23u, &s0, &g0);
    (void)tg14_enqueue(37u, &s1, &g1);
    (void)tg14_enqueue(41u, &s2, &g2);

    tg14_force_reuse(s1, 71u);
    tg14_force_reuse(s1, 89u);
    tg14_force_reuse(s0, 97u);

    if (tg14_accept(s1, g1, &value)) {
        accepted += 1u;
        digest = (digest * 137u) ^ value;
    } else {
        dropped += 1u;
    }
