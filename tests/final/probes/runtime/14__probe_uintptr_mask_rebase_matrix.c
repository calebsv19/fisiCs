...
static unsigned long long fold_mask_rebase(unsigned char* buf, int n) {
    ...
    /* Guardrail: normalize to base-relative offsets.
       Raw pointer low bits are process/address-layout dependent and make
       differential runtime oracles flaky across compilers/invocations. */
