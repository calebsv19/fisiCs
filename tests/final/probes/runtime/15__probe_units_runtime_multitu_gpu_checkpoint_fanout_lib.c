...
static double probe_gpu_checkpoint_fanout_distance_gain(int gpu_tile) {
    static const double gains[3] = {1.0, 0.93, 1.08};
    return gains[gpu_tile % 3];
}

static double probe_gpu_checkpoint_fanout_force_gain(int gpu_tile) {
    static const double gains[3] = {1.0, 1.06, 0.98};
    return gains[gpu_tile % 3];
}
...
ProbeGpuCheckpointFanoutState probe_gpu_checkpoint_fanout_seed(
    ...
    ProbeGpuCheckpointFanoutState state = {
        (speed_mps * dt_s) * probe_gpu_checkpoint_fanout_distance_gain(gpu_tile),
        0.0,
        0.0,
    };
    return state;
...
ProbeGpuCheckpointFanoutState probe_gpu_checkpoint_fanout_push(
    ...
    state.energy_j +=
        (thrust_n * state.distance_m) * probe_gpu_checkpoint_fanout_force_gain(gpu_tile);
