...
static long double axis3_w17_leaf(
    long double a0,
    long double a1,
    long double a2,
    long double a3,
...
static long double axis3_w17_stage(
    long double seed,
    unsigned epoch,
    unsigned frontier,
    unsigned replay,
...
int main(void) {
    ...
    unsigned epoch = 251u;
    unsigned frontier = 31u;
    unsigned replay = 17u;
    long double seed = 1.28125L;
    int i;

    for (i = 0; i < 21; ++i) {
        long double x0 = axis3_w17_stage(
            seed,
            epoch,
            frontier,
            replay,
        ...
        long double x1 = axis3_w17_stage(
            seed + x0 * 0.07421875L,
        ...
        long double x2 = axis3_w17_stage(
            seed + x1 * 0.0546875L,
        ...
        long double x3 = axis3_w17_stage(
        ...
        long double out = x0 * 0.38671875L + x1 * 0.33203125L + x2 * 0.20703125L + x3 * 0.07421875L
                          + (long double)(frontier & 63u) * 0.00341796875L;

        acc ^= qld(out) + (unsigned)(i * 113 + 47);
        acc = rotl32(acc, 9u);
        seed = out * 0.44140625L + seed * 0.3828125L + (long double)i * 0.00341796875L;
        epoch = rotl32(epoch ^ (unsigned)(i * 73 + 23), (unsigned)((i & 7) + 5));
        frontier = rotl32(frontier ^ (unsigned)(i * 59 + 13), (unsigned)((i & 7) + 6));
        replay = rotl32(replay ^ (unsigned)(i * 61 + 7), (unsigned)((i & 7) + 4));
    }
