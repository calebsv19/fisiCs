...
struct Axis3W16Wide axis3_w16_wide_collect(
    ...
    va_list ap;
    struct Axis3W16Wide out;
    static const int lane_map[23] = {1, 2, 0, 1, 2, 0, 2, 1, 0, 2, 0, 1, 2, 1, 0, 1, 2, 0, 2, 1, 0, 2, 1};
    int i;

    out.lo = seed_lo + (long double)(epoch & 63u) * 0.021484375L;
    out.mid = seed_mid + (long double)(frontier & 63u) * 0.01123046875L;
    out.hi = seed_hi - (long double)(replay & 63u) * 0.0010986328125L;
    ...
    for (i = 0; i < count; ++i) {
        ...
        if (kind == 0) {
            ...
            if (lane == 0) {
                out.lo += ldv * (0.2109375L + s);
            } else if (lane == 1) {
                out.mid += ldv * (0.1875L + s);
            } else {
                out.hi += ldv * (0.1796875L + s);
            }
    ...
    out.hi += (long double)(rotl32(replay ^ epoch, frontier & 31u) & 31u) * 0.00537109375L;
    return out;
}
