...
static double probe_gpu_feedback_ladder_force_gain(int gpu_tile) {
    static const double gains[4] = {1.0, 1.04, 0.95, 1.1};
...
ProbeGpuFeedbackLadderState probe_gpu_feedback_ladder_push(
    ...
    state.energy_j +=
        (thrust_n * state.distance_m) * probe_gpu_feedback_ladder_force_gain(gpu_tile);
    return state;
}
...
ProbeGpuFeedbackLadderState probe_gpu_feedback_ladder_merge(
    ...
    ProbeGpuFeedbackLadderState merged = {
