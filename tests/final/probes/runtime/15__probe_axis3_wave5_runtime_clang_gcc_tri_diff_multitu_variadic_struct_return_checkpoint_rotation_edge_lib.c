...
struct Axis3W5Wide axis3_w5_wide_collect(
    ...
    va_list ap;
    struct Axis3W5Wide out;
    static const int lane_map[4] = {2, 1, 0, 2};
    int i;

    out.lo = seed_lo + (long double)(checkpoint & 7u) * 0.125L;
    out.mid = seed_mid;
    out.hi = seed_hi;
    out.tag = checkpoint ^ rotl32(0x9e3779b9u, rotate & 31u);
    ...
    for (i = 0; i < count; ++i) {
        ...
        if (kind == 0) {
            ...
            out.tag ^= q10(ldv) + (unsigned)(i * 17 + 7);
        } else if (kind == 1) {
            ...
            out.tag ^= rotl32(uv + checkpoint + (unsigned)(i * 13 + 5), (unsigned)((i & 7) + 4));
        }
    }
    va_end(ap);

    out.lo += (long double)(rotl32(checkpoint, rotate & 31u) & 31u) * 0.015625L;
    out.hi += (long double)((checkpoint >> 2) & 15u) * 0.0078125L;
    return out;
}
