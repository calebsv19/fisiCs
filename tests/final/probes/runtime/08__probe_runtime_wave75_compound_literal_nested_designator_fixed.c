...
int main(void) {
    struct Volume volume = (struct Volume){
    ...
    printf("%u %u %u %u %u %u %u\n",
           ...
           (unsigned)volume.planes[0].nodes[0][2].raw[0],
           (unsigned)volume.planes[0].nodes[1][0].raw[3],
           (unsigned)volume.planes[1].nodes[1][2].raw[1],
           checksum(&volume));
    return 0;
}
