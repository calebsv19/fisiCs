...
static bool bounded_string_fits(const char *src, size_t dst_size) {
    size_t n = 0;
    if (!src || dst_size == 0) return false;
    n = strlen(src);
    return n < dst_size;
}
...
CoreResult core_trace_session_init(CoreTraceSession *session, const CoreTraceConfig *config) {
    ...
    if (!checked_mul_size(config->sample_capacity, sizeof(CoreTraceSampleF32), &sample_bytes)) {
        return invalid_arg("sample capacity too large");
    }
    if (!checked_mul_size(config->marker_capacity, sizeof(CoreTraceMarker), &marker_bytes)) {
        return invalid_arg("marker capacity too large");
    }
...
CoreResult core_trace_emit_sample_f32(CoreTraceSession *session,
    ...
    if (!bounded_string_fits(lane, CORE_TRACE_LANE_NAME_MAX)) {
...
CoreResult core_trace_emit_marker(CoreTraceSession *session,
    ...
    if (!bounded_string_fits(label, CORE_TRACE_MARKER_LABEL_MAX)) {
...
CoreResult core_trace_import_pack(const char *pack_path, CoreTraceSession *out_session) {
    ...
    if (h.sample_count > (uint64_t)SIZE_MAX) {
        core_pack_reader_close(&r);
        return invalid_arg("sample count too large");
    }
    if (h.marker_count > (uint64_t)SIZE_MAX) {
        core_pack_reader_close(&r);
        return invalid_arg("marker count too large");
    }
    if (!checked_mul_u64(h.sample_count, (uint64_t)sizeof(CoreTraceSampleF32), &sample_bytes)) {
        core_pack_reader_close(&r);
        return invalid_arg("sample payload size overflow");
    }
    if (!checked_mul_u64(h.marker_count, (uint64_t)sizeof(CoreTraceMarker), &marker_bytes)) {
        core_pack_reader_close(&r);
        return invalid_arg("marker payload size overflow");
    }
